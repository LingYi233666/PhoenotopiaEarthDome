...
local SMOKE_COLOUR_ENVELOPE_NAME = "galeboss_ruinforce_projectile_dark_vfx_smoke_colourenvelope"
local SMOKE_STATIC_COLOUR_ENVELOPE_NAME = "galeboss_ruinforce_projectile_dark_vfx_smoke_static_colourenvelope"
local SMOKE_SCALE_ENVELOPE_NAME = "galeboss_ruinforce_projectile_dark_vfx_smoke_scaleenvelope"
local SMOKE_STATIC_SCALE_ENVELOPE_NAME = "galeboss_ruinforce_projectile_dark_vfx_smoke_static_scaleenvelope"
...
local function fn()
    ...
    local num_to_emit_static = 0
    local num_to_emit_tail = 0

    EmitterManager:AddEmitter(inst, nil, function()
        num_to_emit_static = num_to_emit_static + FRAMES * 7
        num_to_emit_tail = num_to_emit_tail + FRAMES * 34

        while num_to_emit_static > 1 do
            emit_smoke_static_fn(effect, sphere_static_emitter)
            num_to_emit_static = num_to_emit_static - 1
        end

        while num_to_emit_tail > 1 do
            emit_smoke_fn(effect, sphere_emitter)
            num_to_emit_tail = num_to_emit_tail - 1
        end
...
return Prefab("galeboss_ruinforce_projectile_dark_vfx", fn, assets)
