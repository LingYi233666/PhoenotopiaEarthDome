...
local COLOUR_ENVELOPE_NAME_SMOKE = "typhon_mimic_multiply_vfx_colourenvelope_smoke"
local SCALE_ENVELOPE_NAME_SMOKE = "typhon_mimic_multiply_vfx_scaleenvelope_smoke"
local COLOUR_ENVELOPE_NAME_EMBER = "typhon_mimic_multiply_vfx_colourenvelope_ember"
local SCALE_ENVELOPE_NAME_EMBER = "typhon_mimic_multiply_vfx_scaleenvelope_ember"
local COLOUR_ENVELOPE_NAME_POINT = "typhon_mimic_multiply_vfx_colourenvelope_point"
local SCALE_ENVELOPE_NAME_POINT = "typhon_mimic_multiply_vfx_scaleenvelope_point"
...
local function InitEnvelope()
    ...
    -- for i = 0, 4 do
    --     if i % 2 == 0 then
    --         table.insert(ember_colours, { i * 0.2, IntColour(255, 255, 255, 255) })
    --     else
    ...
    EnvelopeManager:AddVector2Envelope(
        ...
        {
            { 0, { ember_max_scale * 0.075, ember_max_scale } },
            { 1, { ember_max_scale * 0.075 * 0.1, ember_max_scale * 0.1 } },
        }
    )
...
local function emit_ember_fn(effect, sphere_emitter, adjust_vec)
    ...
    local px, py, pz = sphere_emitter()
    local vx, vy, vz = (Vector3(px, py, pz):GetNormalized() * GetRandomMinMax(0.2, 0.3)):Get()
    -- vy = math.abs(vy)
...
local function fn()
    ...
    local smoke_sphere_emitter = CreateSphereEmitter(.15)
    local point_sphere_emitter = CreateSphereEmitter(.5)
    local ember_sphere_emitter = CreateSphereEmitter(1.0)

    local num_to_emit_smoke = 1
    local num_to_emit_spark = 1
    local num_to_emit_ember = 1
    ...
    EmitterManager:AddEmitter(inst, nil, function()
        ...
        num_to_emit_smoke = num_to_emit_smoke + 0.66
        num_to_emit_spark = num_to_emit_spark + 1
        num_to_emit_ember = num_to_emit_ember + 0.66
    end)

    return inst
end

return Prefab("typhon_mimic_multiply_vfx", fn, assets)
