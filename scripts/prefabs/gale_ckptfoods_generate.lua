...
local function MakePreparedFood(data)
    ...
    local function fn()
        ...
        if data.health or data.hunger or data.foodtype or data.secondaryfoodtype
            or data.sanity or data.temperature or data.temperatureduration
            or data.nochill or data.spice or data.oneatenfn then
        ...
        if STRINGS.GALE_ITEM_DESC[data.name:upper()] then
...
for k, recipe in pairs(require("gale_ckptfoods1")) do
    table.insert(prefs, MakePreparedFood(recipe))

    if recipe.test then
        AddCookerRecipe("cookpot", recipe, cooking.IsModCookerFood(k))
        -- AddCookerRecipe("gale_cookpot", recipe, cooking.IsModCookerFood(k))
        -- AddCookerRecipe("gale_cookpot_duplicate", recipe, cooking.IsModCookerFood(k))
    ...
    --     if TheCookbook and not TheCookbook:IsUnlocked(k) then
    --         TheCookbook:AddRecipe(k,recipe.unlock)
    --     end
    -- end
end
