    ...
    inst:StartThread(function()
        ...
        while rad <= max_rad do
            ...
            if new_rad - old_rad >= 1.5 then
                for emit_rad=old_rad,new_rad, 30 * FRAMES do
                    ...
                    for deg = start_deg,start_deg+360-dd,dd do
                        SpawnAt("electrichitsparks",inst,nil,Vector3(math.cos(deg * DEGREES),0,math.sin(deg * DEGREES)) * emit_rad)
            ...
            for emit_rad=old_rad,new_rad, 8 * FRAMES do
                ...
                local start_deg = math.random() * 360
                for deg = start_deg,start_deg+360-dd,dd do
                    table.insert(points,inst:GetPosition() + Vector3(math.cos(deg * DEGREES),0,math.sin(deg * DEGREES)) * emit_rad)
                end
                ...
                if emit_rad >= 4 and emit_rad == old_rad then
                    ...
                    for deg = start_deg,start_deg+360-dd,dd do
                        ...
                        SpawnPrefab("gale_lightningfx"):Emit(inst:GetPosition() + offset * math.min(1,emit_rad),inst:GetPosition() + offset  * emit_rad,scale)
                    end
