...
local COLOUR_ENVELOPE_NAME = "gale_carry_charge_vfx_colourenvelope"
local SCALE_ENVELOPE_NAME = "gale_carry_charge_vfx_scaleenvelope"
...
local function emit_sparkle_fn(effect, sphere_emitter)
    local vx, vy, vz = .003 * UnitRand(), GetRandomMinMax(0.007,0.01), .003 * UnitRand()
...
local function fn()
    ...
    effect:SetDragCoefficient(0,0.003)
    effect:SetSortOrder(0, 0)
    effect:SetSortOffset(0, 2)

    local sphere_emitter = CreateSphereEmitter(.33)

    local num_to_emit = 0
    EmitterManager:AddEmitter(inst, nil, function()
        num_to_emit = num_to_emit + 0.15
...
return Prefab("gale_carry_charge_vfx", fn, assets)
