local GLITCH_TEXTURE = resolvefilepath("fx/athetos_treasure_tree.tex")

local ADD_SHADER = "shaders/vfx_particle_add.ksh"

local COLOUR_ENVELOPE_NAME = "athetos_treasure_explode_tree_vfx_colourenvelope"
local SCALE_ENVELOPE_NAME = "athetos_treasure_explode_tree_vfx_scaleenvelope"
...
local function fn()
    ...
    effect:SetUVFrameSize(0, 1/16, 1/16)
    ...
    EmitterManager:AddEmitter(inst, nil, function()
        if not spawned then
            for x = 5,10 do
                for y=0,15 do
                    emit_glitch_fn(effect, box_emitter,x/16,y/16)
                end
            end
            spawned = true 
        end
...
return Prefab("athetos_treasure_explode_tree_vfx", fn, assets)
