...
function GaleBlasterJammed:GetPercent()
    ...
    if self.inst.components.temperature and self.inst.components.temperature:IsOverheating() then
        ...
        percent = percent + Remap(self.inst.components.temperature:GetCurrent(),begin_heat_temp,max_heat_temp,0,self.temperature_facter)
    end

    if self.inst.components.finiteuses and self.inst.components.finiteuses:GetPercent() < self.finiteuses_thres then
        percent = percent + Remap(self.inst.components.finiteuses:GetPercent(),0,self.finiteuses_thres,self.finiteuses_facter,0)
    end
