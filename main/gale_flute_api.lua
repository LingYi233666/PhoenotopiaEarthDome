...
local flute_sg_server = State {
    ...
    onexit = function(inst)
        ...
        SendModRPCToClient(CLIENT_MOD_RPC["gale_rpc"]["remove_areabgm_pause_source"], inst.userid, inst,
                           "gale_flute_play")
    end,
...
AddModRPCHandler("gale_rpc", "melody_play", function(inst, name)
    if inst.sg and inst.sg:HasStateTag("gale_flute_play") then
        ...
        if inst.components.gale_flute_buffed then
            local delighted_plants = inst.components.gale_flute_buffed:DelightPlants()

            -- Trigger special song buff
            local result = inst.components.gale_flute_buffed:TryTrigger(name)
            local announce_msg = ""
            if result == nil then
                announce_msg = string.format(STRINGS.GALE_MELODIES.TRIGGER_NORMAL,
                                             STRINGS.GALE_MELODIES.NAME[name:upper()])
            elseif result == false then
                announce_msg = string.format(STRINGS.GALE_MELODIES.TRIGGER_FAIL[name:upper()],
                                             inst.components.gale_flute_buffed:GetCD(name))
            else
            ...
            SendModRPCToClient(CLIENT_MOD_RPC["gale_rpc"]["announce"], inst.userid, announce_msg, "item_drop", 202 / 255,
                               174 / 255, 118 / 255, 255 / 255)

            if #delighted_plants > 0 then
                local message = string.format(STRINGS.GALE_MELODIES.TEND_TO_PLANTS, #delighted_plants)
                SendModRPCToClient(CLIENT_MOD_RPC["gale_rpc"]["announce"], inst.userid, message, "item_drop",
                                   202 / 255,
                                   174 / 255, 118 / 255, 255 / 255)
            end
        end
...
AddClientModRPCHandler("gale_rpc", "popup_flute_fx", function(singer, image_name)
    if ThePlayer.HUD then
        ...
        ThePlayer.HUD.popupstats_root:AddChild(GalePopupFlute(ThePlayer, singer:GetPosition() + Vector3(0, 1, 0), 1.5,
                                                              image_name))
    end
end)
