...
AddComponentPostInit("playercontroller", function(self)
    ...
    self.GetRightMouseAction = function(self, ...)
        ...
        if equipitem and equipitem.components.aoetargeting then
            local position = TheInput:GetWorldPosition()
            local target = TheInput:GetWorldEntityUnderMouse()
            local old_right = self.inst.components.playeractionpicker:GetRightClickActions(position, target)[1]
...
AddComponentPostInit("playeractionpicker", function(self)
    ...
    self.GetRightClickActions = function(self, position, target, ...)
        ...
        if actions == nil or #actions <= 0 or actions[1].action == ACTIONS.CASTAOE then
            local equipitem = self.inst.replica.inventory:GetEquippedItem(EQUIPSLOTS.HANDS)
            if equipitem and target and equipitem.components.aoetargeting ~= nil then
                ...
                if scene_action then
                    if (scene_action.action.priority >= 0 and scene_action.action.rmb)
                        or (scene_action.action == ACTIONS.RUMMAGE) then
...
local function CanUseCharge(inst)
    local weapon = (inst.components.combat and inst.components.combat:GetWeapon()) or
    ...
    local is_sg_gale_carry_charge_pst = (inst.sg and inst.sg:HasStateTag("gale_carry_charge_pst"))
    return weapon and weapon:HasTag("gale_chargeable_weapon") and is_valid_user and not is_riding
end
...
local function ServerGetAttackSG(inst, action)
    ...
    if not (inst.sg:HasStateTag(attack_tag) and action.target == inst.sg.statemem.attacktarget or inst.components.health:IsDead()) then
        if not is_riding and inst.components.gale_skill_electric_punch
        ...
        if weapon then
            ...
            if not is_riding then
                ...
                elseif weapon:HasTag("gale_blaster") then
                    if weapon:HasTag("gale_blaster_jammed") or weapon:HasTag("gale_blaster_out_of_ammo") then
                        return
                    end
                    if weapon.prefab == "msf_silencer_pistol" then
                        return "galeatk_pistol_earlier_14_remove_attacktag_at_18"
                    end
...
local function ClientGetAttackSG(inst, action)
    ...
    if not (inst.sg:HasStateTag("attack") and action.target == inst.sg.statemem.attacktarget or IsEntityDead(inst)) then
        if not is_riding and inst.replica.gale_skill_electric_punch
...
local function PushSpecialAtkEvent(inst, other_data)
    local target = inst.sg.statemem.attacktarget or inst.components.combat.target
...
local function ChargingRunOrStop(inst, ismastersim)
    local cmp_gale_weaponcharge = ismastersim and inst.components.gale_weaponcharge or inst.replica.gale_weaponcharge
    ...
    local moving_vec = inst.components.gale_control_key_helper:GetMovingDirectVector()
    local should_move = moving_vec and moving_vec:Length() > 0
        and cmp_gale_weaponcharge:AtkPressed(is_free_charge)
        and inst.sg:GetTimeInState() >= 3 * FRAMES

    if should_move then
        inst:ForceFacePoint(inst:GetPosition() + moving_vec)
    end
...
table.insert(SERVER_SG, State {
    ...
    tags = { "charging_attack", "charging_attack_pre", "moving", "running", "autopredict", },

    onenter = function(inst)
        inst:AddTag("charging_attack_pre")

        inst.components.locomotor:SetExternalSpeedMultiplier(inst, "gale_charging_attack_pre", 0.5)
        ...
        inst.sg.statemem.is_free_charge = buffaction and buffaction.action == ACTIONS.GALE_FREE_CHARGE
    ...
    onexit = function(inst)
        ...
        inst.components.locomotor:RemoveExternalSpeedMultiplier(inst, "gale_charging_attack_pre")
    end,
})
...
table.insert(SERVER_SG, State {
    ...
    timeline = {
        TimeEvent(FRAMES, function(inst)
            ...
            if cur_buffered_action == nil or (cur_buffered_action ~= nil and cur_buffered_action ~= inst.sg.statemem.buffaction_storage) then
...
table.insert(SERVER_SG, State {
    ...
    timeline = {
        ...
        TimeEvent(17 * FRAMES, function(inst)
            ...
            inst.sg.statemem.tail_fx = inst:SpawnChild("gale_sky_striker_blade_fire_tail")
            ...
            -- end)
            inst.sg.statemem.dash_task = inst:DoPeriodicTask(0, function()
        ...
        TimeEvent(24 * FRAMES, function(inst)
            if inst.sg.statemem.addition_attack then
                inst.sg:GoToState("gale_fire_dash_addition", inst.sg.statemem.fxs)
            else
                inst.sg:GoToState("idle", true)
            end
        end),
...
table.insert(SERVER_SG, State {
    ...
    onenter = function(inst)
        ...
        local charge_sound = weapon and weapon.charge_sound or "dontstarve/wilson/attack_weapon"
    ...
    events =
    {
        EventHandler("animqueueover", function(inst)
            if inst.AnimState:AnimDone() and inst.AnimState:IsCurrentAnimation("atk") then
                inst.sg:GoToState("idle")
            end
        end),
    },
...
table.insert(SERVER_SG, State {
    ...
    timeline = {
        ...
        TimeEvent(13 * FRAMES, function(inst)
            ...
            local world_pos = Vector3(inst.entity:LocalToWorldSpace(local_pos:Get()))
            ...
            inst:SpawnChild("gale_explode_ray_yellow_vfx").Transform:SetPosition(local_pos:Get())
...
table.insert(SERVER_SG, State {
    ...
    onenter = function(inst)
        ...
        inst.components.locomotor:SetExternalSpeedMultiplier(inst, "gale_lightning_roll", 1.2)
    ...
    onupdate = function(inst)
        local moving_dir = inst.components.gale_control_key_helper:GetMovingDirectVector()
...
table.insert(SERVER_SG, State {
    ...
    onenter = function(inst)
        ...
        inst.sg.statemem.attack = (bufferedaction and bufferedaction.action == ACTIONS.ATTACK)
    end,
    timeline = {
    ...
    sg.events["attacked"].fn = function(inst, data)
        if not inst.components.health:IsDead() then
            ...
            elseif inst.sg.currentstate.name == "gale_tired_low_stamina" then
                ...
                return
            elseif inst.sg.currentstate.name == "gale_tired_low_stamina_knockback" then
                inst.sg:GoToState("gale_tired_low_stamina_knockback")
                return
            elseif inst.sg.currentstate.name == "gale_tired_low_stamina_knockback_pst" then
...
AddModRPCHandler("gale_rpc", "gale_weaponcharge_btn", function(inst, control, pressed)
...
AddModRPCHandler("gale_rpc", "update_mouse_position", function(inst, x, z)
    if inst.components.gale_control_key_helper then
        inst.components.gale_control_key_helper:SetMousePosition(Vector3(x, 0, z))
    end
end)
...
AddModRPCHandler("gale_rpc", "update_moving_direct_vector", function(inst, x, z)
    if inst.components.gale_control_key_helper then
        inst.components.gale_control_key_helper:SetMovingDirectVector(Vector3(x, 0, z))
    end
end)
...
TheInput:AddGeneralControlHandler(function(control, pressed)
    if ThePlayer and ThePlayer:HasTag("gale_weaponcharge") then
        if table.contains(atk_btns, control) then
            SendModRPCToServer(MOD_RPC["gale_rpc"]["gale_weaponcharge_btn"], control, pressed)
        end
...
AddAction("GALE_FREE_CHARGE", "GALE_FREE_CHARGE", function(act)
    if CanUseCharge(act.doer) then
        ...
        if weapon and act.doer.components.gale_weaponcharge then
            local pos = act.doer.components.gale_control_key_helper:GetMousePosition()
            local undermouse = act.doer.components.gale_control_key_helper:GetEntityUnderMouse()
...
AddComponentAction("POINT", "gale_chargeable_weapon", function(inst, doer, pos, actions, right, target)
    if right and not doer:HasTag("charging_attack_pre") and CanUseCharge(doer) and not doer._just_use_carry_charge then
        table.insert(actions, ACTIONS.GALE_FREE_CHARGE)
    end
end)

AddComponentAction("EQUIPPED", "gale_chargeable_weapon", function(inst, doer, target, actions, right)
    if right and not doer:HasTag("charging_attack_pre") and CanUseCharge(doer) and not doer._just_use_carry_charge then
        table.insert(actions, ACTIONS.GALE_FREE_CHARGE)
    end
end)
...
AddAction("GALE_FREE_SHOOT", "GALE_FREE_SHOOT", function(act)
    ...
    if pos and act.invobject and act.invobject:IsValid()
        and act.invobject.components.equippable
        and act.invobject.components.equippable:IsEquipped()
        and act.invobject.components.inventoryitem
        and act.invobject.components.inventoryitem.owner == act.doer
        and act.invobject.components.gale_blaster_freeshoot
    then
        -- act.doer:ForceFacePoint(pos)
        return act.invobject.components.gale_blaster_freeshoot:FreeShoot(act.doer, pos)
    end
end)
...
-- inst, doer, pos, actions, right, target
AddComponentAction("POINT", "gale_blaster_freeshoot", function(inst, doer, pos, actions, right, target)
    if right then
        table.insert(actions, ACTIONS.GALE_FREE_SHOOT)
    end
end)

AddComponentAction("EQUIPPED", "gale_blaster_freeshoot", function(inst, doer, target, actions, right)
    if right then
        table.insert(actions, ACTIONS.GALE_FREE_SHOOT)
    end
end)
...
AddStategraphActionHandler("wilson", ActionHandler(ACTIONS.GALE_FREE_SHOOT, function(inst)
    return "galeatk_quick_pistol"
end))
AddStategraphActionHandler("wilson_client", ActionHandler(ACTIONS.GALE_FREE_SHOOT, function(inst)
    return "galeatk_quick_pistol"
end))
...
AddClientModRPCHandler("gale_rpc", "popup_number", function(val, x, y, z)
    if type(val) == "number" then
        local colour = val > 0 and { 0 / 255, 255 / 255, 78 / 255, 1 } or { 255 / 255, 80 / 255, 40 / 255, 1 }
        ...
        if math.abs(val) >= 1 then
            ...
            ThePlayer.HUD:ShowPopupNumber(val, size, Vector3(x, y, z), 40, colour, burst)
        else
            ThePlayer.HUD:ShowPopupNumber(string.format("%.1f", math.abs(val)), size, Vector3(x, y, z), 40, colour, burst)
        end
    elseif type(val) == "string" then
        ThePlayer.HUD:ShowPopupNumber(val, 32, Vector3(x, y, z), 40, { 0 / 255, 0 / 255, 200 / 255, 1 }, false)
    end
end)
            ...
            if apply_amount and data_amount and data_amount ~= 0 then
                ...
                for _, v in pairs(AllPlayers) do
                    if v and v:IsValid() and v:IsNear(inst, 40) then
                        SendModRPCToClient(CLIENT_MOD_RPC["gale_rpc"]["popup_number"], v.userid, apply_amount, x, y, z)
                    end
                end
            end
        end
...
GLOBAL.GaleModAddKnockbackSG = function(sgname, add_data)
    ...
    local knockback_sg = State {
        ...
        onenter = function(inst, data)
            ...
            if not data.is_dead then
                inst.AnimState:PlayAnimation(CheckFuncVal(add_data.knockback_anim, inst) or "hit")
            end
            ...
            if data ~= nil then
                if data.radius ~= nil and data.knocker ~= nil and data.knocker:IsValid() then
                    ...
                    inst.sg.statemem.speed = (CheckFuncVal(add_data.speed, inst) or (data.strengthmult or 1) * 12) * k
                    inst.sg.statemem.dspeed = CheckFuncVal(add_data.dspeed, inst) or 0
        ...
        onupdate = function(inst)
            if inst.sg.statemem.speed ~= nil and not inst.sg.statemem.sinked then
                ...
                if inst.sg.statemem.speed < 0 then
                    inst.sg.statemem.dspeed = inst.sg.statemem.dspeed + (CheckFuncVal(add_data.vec_acc, inst) or 0.075)
                else
    ...
    AddStategraphEvent(sgname, EventHandler("knockback", function(inst, data)
        ...
        if data.is_dead and add_data.dont_knockback_ondeath then
            return
        end
...
GaleModAddKnockbackSG("frog", { knockback_anim = "fall_idle", hoc_hit = true, vec_acc = 0.0075 })
GaleModAddKnockbackSG("squid", { hoc_hit = true, vec_acc = 0.0075 })
GaleModAddKnockbackSG("mole", { knockback_anim = "stunned_loop", hoc_hit = true, vec_acc = 0.0075 })
...
GaleModAddKnockbackSG("bearger", { knockback_anim = "standing_hit", speed = 3, dont_knockback_ondeath = true })
...
GaleModAddKnockbackSG("mossling", { knockback_anim = "meep", hoc_hit = true, vec_acc = 0.0075 })

GaleModAddKnockbackSG("bishop")
GaleModAddKnockbackSG("knight")
GaleModAddKnockbackSG("rook")
GaleModAddKnockbackSG("powdermonkey", { knockback_anim = "hit", hoc_hit = true, vec_acc = 0.0075 })
