...
local skilltree_Assets = {
    ...
    Asset("IMAGE", "images/ui/skill_slot/actioned_charge.tex"),
    Asset("ATLAS", "images/ui/skill_slot/actioned_charge.xml"),
    ...
    Asset("ATLAS", "images/ui/skill_slot/hyper_burn.xml"),
...
AddReplicableComponent("gale_skill_electric_punch")
...
GLOBAL.GALE_SKILL_NODES = {
    ...
    DESTRUCT_TABLE = GaleNode({
        ui_pos = Vector3(0, 2),
        ingredients = {
            Ingredient("athetos_mushroom_cap", 1, "images/inventoryimages/athetos_mushroom_cap.xml"),
        },
        OnLearned = function(inst, is_load)
            inst:AddTag("gale_destruct_item_table_builder")
        end,
        OnForget = function(inst)
            inst:RemoveTag("gale_destruct_item_table_builder")
        end,
    }),
    ...
    ACTIONED_CHARGE = GaleNode({
        ui_pos = Vector3(2, 2),
        ingredients = {
            Ingredient("athetos_mushroom_cap", 4, "images/inventoryimages/athetos_mushroom_cap.xml"),
        },
        OnPressed = function(inst, x, y, z, ent)
            if not inst.sg:HasStateTag("dead")
                and not IsEntityDeadOrGhost(inst)
            ...
            then
                if inst.components.health.currenthealth <= 5 then
                    SendModRPCToClient(CLIENT_MOD_RPC["gale_rpc"]["announce"], inst.userid,
                        STRINGS.GALE_SKILL_CAST.FAILED.INSUFFICIENT_HEALTH)
                    return
                end
                ...
                if inst.sg:HasStateTag("charging_attack_pre") then
                    if not inst.components.gale_weaponcharge:IsComplete() then
                        inst.components.gale_weaponcharge:Complete()
                        casted = true
                    end
                elseif not inst.sg:HasStateTag("busy") and GaleCondition.GetCondition(inst, "condition_carry_charge") == nil then
                    GaleCondition.AddCondition(inst, "condition_carry_charge")
                    casted = true
                end

                if casted then
                    -- SpawnPrefab("gale_hit_color_adder_green"):SetTarget(inst)
                    SpawnPrefab("gale_hit_color_adder"):SetTarget(inst)

                    inst.components.gale_stamina:DoDelta(10)
                    inst.components.health:DoDelta(-5)
                    -- Play sfx here
                    -- inst.SoundEmitter:PlaySound("")
                    inst.SoundEmitter:PlaySound("gale_sfx/skill/actioned_charge")
                    inst.SoundEmitter:PlaySound("gale_sfx/skill/actioned_charge2")
                end
            end
        end,
    }),
    ...
    COMBAT_LEAP = GaleNode({
        ...
        OnPressed = function(inst, x, y, z, ent)
            ...
            then
                local weapon = inst.components.combat:GetWeapon()
                if not (weapon and weapon.components.gale_helmsplitter) then
                    return
                end
                ...
                local deltapos = targetpos - inst:GetPosition()
                if deltapos:Length() > 6 then
                    deltapos = deltapos:GetNormalized() * 6
                    targetpos = inst:GetPosition() + deltapos
                end


                local last_cast_time = inst.components.gale_skiller.skillmem.LastCombatLeapTime

                if last_cast_time == nil or GetTime() - last_cast_time >= 3 then
                    inst.sg:GoToState("gale_skill_combat_leap", {
                        weapon = weapon,
                        targetpos = targetpos,
                    })
                    inst.components.gale_stamina:DoDelta(-10)

                    inst.components.gale_skiller.skillmem.LastCombatLeapTime = GetTime()
                end
            end
        end,
    }),
    ...
    HYPER_BURN = GaleNode({
        ...
        OnPressed = function(inst, x, y, z, ent)
            ...
            if inst.sg.currentstate.name ~= "gale_skill_hyperburn_pre" then
                local cooldown = 2
                local last_cast_time = inst.components.gale_skiller.skillmem.LastHyperBurnTime or 0
            ...
            if cast_success then
                SendModRPCToClient(CLIENT_MOD_RPC["gale_rpc"]["play_clientside_sound"], inst.userid,
                    "gale_sfx/skill/ping_spawn_sfx", true, true)
...
GLOBAL.GALE_SKILL_NODES.CARRY_CHARGE:AddChilds({
    ...
    GLOBAL.GALE_SKILL_NODES.ACTIONED_CHARGE,
})
...
GLOBAL.GALE_SKILL_TREE.SCIENCE.root:AddChilds({
    ...
    GLOBAL.GALE_SKILL_NODES.DESTRUCT_TABLE,
})
...
GLOBAL.GALE_SKILL_NODES.KINETIC_BLAST:AddChilds({
    GLOBAL.GALE_SKILL_NODES.HYPER_BURN,
    GLOBAL.GALE_SKILL_NODES.ELECTRIC_PUNCH,
})
...
AddModRPCHandler("gale_rpc", "press_space_and_carry_charge", function(inst)
    if inst.components.gale_weaponcharge:IsComplete() and inst.components.gale_skiller:IsLearned("carry_charge") then
        ...
        GaleCondition.AddCondition(inst, "condition_carry_charge")
        inst.SoundEmitter:PlaySound("gale_sfx/battle/p1_weapon_charge_carry")
...
AddStategraphState("wilson", State {
    ...
    ontimeout = function(inst)
        inst.sg.statemem.success_exit = true
        inst.sg:GoToState("gale_skill_hyperburn")
    end,

    onexit = function(inst)
        if not inst.sg.statemem.success_exit then
            inst.components.gale_skill_hyperburn:ClearList()
        end
    end,
})
...
-- ThePlayer.sg:GoToState("gale_skill_combat_leap",{targetpos=TheInput:GetWorldPosition(),weapon=ThePlayer.components.combat:GetWeapon()})
AddStategraphState("wilson", State {
...
AddComponentPostInit("combat", function(self)
    local old_GetWeapon = self.GetWeapon
    self.GetWeapon = function(self, ...)
        local punch_weapon = nil
        if self.inst.components.gale_skill_electric_punch
            and self.inst.components.gale_skill_electric_punch:IsEnabled()
            and self.inst.components.gale_skill_electric_punch.force_punch_weapon then
            punch_weapon = self.inst.components.gale_skill_electric_punch:GetWeapon()
        end
        return punch_weapon or old_GetWeapon(self, ...)
    end
end)
