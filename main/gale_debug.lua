...
function GLOBAL.HyperBurn(attacker, start_pos, end_pos)
    ...
    -- attacker.SoundEmitter:PlaySound("dontstarve/common/fireAddFuel")
    attacker.SoundEmitter:PlaySound("gale_sfx/battle/active_grenade_fire")
    -- attacker.SoundEmitter:PlaySound("dontstarve/common/blackpowder_explo")
    ...
    if onocean then
        ...
        ShakeAllCameras(CAMERASHAKE.FULL, .35, .02, 1, end_pos, 40)

        -- for i = 1, math.random(2, 3) do
        --     local offset = FindWalkableOffset(end_pos, math.random() * TWOPI, GetRandomMinMax(0, 2), 10, nil, true, nil,
        --                                       false, true)
        --     if offset then
        --         local fire        = SpawnAt("houndfire", end_pos, nil, offset + Vector3(0, GetRandomMinMax(0.5, 1.5), 0))
        --         local offset_norm = offset:GetNormalized()
        --         local vel         = offset_norm * 1 + towards_norm * 2.5
        --         fire.Physics:SetVel(vel:Get())
        --     end
        -- end

        -- local fire = SpawnAt("houndfire", end_pos, nil, Vector3(0, GetRandomMinMax(0.5, 1.5), 0))
        -- fire.Physics:SetVel((towards_norm * 0.5):Get())

        GaleCommon.AoeForEach(attacker, end_pos, 3, nil, { "INLIMBO", "FX" }, nil,
                                  ...
                                  if target.components.combat and target.components.health then
                                      if attacker.components.combat:CanTarget(target) and
                                          ...
                                          if not target.components.health:IsDead() then
                                              target.components.combat:GetAttacked(attacker, GetRandomMinMax(16, 20))
                                          end

                                          if not target.components.health:IsDead() then
                                              target.components.health:DoFireDamage(GetRandomMinMax(16, 20), attacker,
                                                                                    true)
                                          end
                                      end
